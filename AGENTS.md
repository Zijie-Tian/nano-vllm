...
| [`docs/prerope_policy_design.md`](docs/prerope_policy_design.md) | PreRoPE lazy-RoPE policy 设计：pre-RoPE KV cache 语义、amplitude/phase 不变量、数据流，以及稀疏策略的数学基础。 |
...
| [`docs/chunked_prefill_timing_sync_default_off.md`](docs/chunked_prefill_timing_sync_default_off.md) | 记录 `NANOVLLM_CHUNKED_TIMING_SYNC` 默认关闭的变更、计时语义影响，以及 GPU1 上 5-sample RULER 通过验证。 |
...
| [`docs/triattention_prefill_extension_design.md`](docs/triattention_prefill_extension_design.md) | TriAttention prefill extension: four independent design schemes (TriHead, TriBand, TriCausal, TriCache) extending decode-centric TriAttention ideas to prefill phase. |
