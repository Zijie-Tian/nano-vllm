...
from nanovllm.ops.tvm_qgemm.utils.math_utils import compute_sqnr, nmse, compute_error, print_binary
from nanovllm.ops.tvm_qgemm.utils.quant import (
    quantize_weight_per_tensor, dequantize_weight_per_tensor, 
    quantize_weight_per_group, dequantize_weight_per_group
)
...
print(f"Overall weight stats: min={weight.min():.3f}, max={weight.max():.3f}, mean={weight.mean():.3f}, std={weight.std():.3f}")
print(f"Activation stats: min={activation.min():.3f}, max={activation.max():.3f}, mean={activation.mean():.3f}, std={activation.std():.3f}\n")

strategies_to_test = [QUANT_STRATEGY] if not COMPARE_ALL_STRATEGIES else list(QuantizationStrategy)
...
for test_strategy in strategies_to_test:
    ...
    if test_strategy == QuantizationStrategy.PER_TENSOR:
        print(f"Using PER-TENSOR quantization strategy")
        weight_quant, scales, zp = quantize_weight_per_tensor(weight, bits=bits, sym=sym)
        dequantized_weight = dequantize_weight_per_tensor(weight_quant, scales, zero_point=zp)
    elif test_strategy == QuantizationStrategy.PER_GROUP:
        print(f"Using PER-GROUP quantization strategy (group_size={group_size})")
        weight_quant, scales, zp = quantize_weight_per_group(weight, bits=bits, group_size=group_size, sym=sym)
    ...
    if m_groups == -1:
        Adq = Aref.T.reshape(K // group_size, group_size, M // bits).astype("float16") - (2 ** (bits - 1))
    ...
    def preprocessor_reference(B, act_group_size, g, dtype, out_dtype):
        ...
        ils = np.vectorize(lambda s: 1.0 / s if s != 0 else 0)(lut_scales).astype(out_dtype)
        qlut = np.rint(
    ...
    def qgemm_reference(A, QLUT, LUT_Scales, LUT_Biases, scales, bits, g, group_size, m_groups, simd_n_in, simd_n_out, bm, kfactor):
        ...
        A_unpacked = A.reshape(M_total // bm, K_total // g // kfactor, bm // _ngroups_per_elem // simd_n_in, kfactor, simd_n_in)
        A_unpacked = np.concatenate([(A_unpacked >> (g * ng)) & ((1 << g) - 1) for ng in range(_ngroups_per_elem)], axis=-1)
        if zero_point:
            scales_p = scales.reshape(M_total // bm, K_total // group_size, bm // bits // simd_n_out, 2, simd_n_out)
        else:
            scales_p = scales.reshape(M_total // bm, K_total // group_size, bm // bits // simd_n_out, simd_n_out)
        for n in range(N):
            for k in range(K_total // g):
                for m in range(M_total):
                    ...
                    if m_groups == -1:
                        s = scales_p[mo, k * g // group_size, s_mi, 0, s_e] if zero_point else scales_p[mo, k * g // group_size, s_mi, s_e]
                    else:
                    ...
                    if (((k * g) % act_group_size) == 0) and ((((m % bm) // simd_n_out) % bits) == 0):
                        ...
                        if zero_point:
                            cbits[n, m] += LUT_Biases[n, k * g // act_group_size] * (1 / alphas[0]) * scales_p[mo, k * g // group_size, s_mi, 1, s_e]
        return (cbits.reshape((N, M_total // simd_n_out // bits, bits, simd_n_out))
                .transpose(0, 1, 3, 2).dot(np.array(alphas, dtype=out_dtype)).reshape((N, M_total // bits)))

    C = qgemm_reference(A_t, QLUT, LUT_Scales, LUT_Biases, Scales_t, bits, g, group_size, m_groups, simd_n_in, simd_n_out, bm, kfactor)
