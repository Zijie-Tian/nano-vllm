...
def load_real_data(ctx_len='32k'):
    ...
    if not os.path.exists(path):
        print(f"Data not found at {path}. Please symlink /home/zijie/Code/COMPASS/results/kvcache-rope/glm-4-9b to tests/data/real_kvcache")
...
def run_bench_on_data(q, k, v, lambda_val, iters=5):
    ...
    total_compute_density = 0
    total_required_kv_density = 0
    total_time = 0
    ...
    for _ in range(iters):
        ...
        for start, end, is_causal in kv_ranges:
            ...
            mask = torch.ones((grid_0, grid_1, num_kv_sub), device='cuda', dtype=torch.int8)
            if is_causal:
                # Apply block-level causal mask
                for q_idx in range(grid_0):
                    q_end_pos = (q_idx + 1) * BLOCK_M
                    for kv_idx in range(num_sub := num_kv_sub):
...
def main():
    ...
    for l in lambdas:
        res = run_bench_on_data(q, k, v, l)
        print(f"{res['lambda']:8.3f} | {res['compute_density']:15.2f}% | {res['required_kv_density']:15.2f}% | {res['time_ms']:9.2f}")
