"""
PreRoPE full-attention policy.

This policy keeps FULL attention semantics while changing where RoPE is applied:
- model layers pass raw pre-RoPE Q/K into Attention
- KV cache / CPU offload persist raw pre-RoPE K
- RoPE is applied lazily inside this policy right before attention kernels run
"""
...
class PreRoPEPolicy(SparsePolicy):
    """
    FULL attention semantics with lazy RoPE application inside policy methods.

    The critical semantic difference versus FullAttentionPolicy is that
    ``apply_rope_in_attention=True`` makes Attention receive raw pre-RoPE Q/K,
    so KV cache storage and CPU offload both persist pre-RoPE K states.
    """
    ...
    def _get_rope_context(self):
        ...
        if context.positions is None or context.rotary_emb is None:
            raise RuntimeError(
                "PreRoPEPolicy requires context.positions and context.rotary_emb"
            )
    ...
    def print_density_stats(self) -> None:
        ...
        logger.info(
            f"[PreRoPE Policy] Density Stats: chunks={stats['num_chunks']}, "
    ...
    def compute_decode(
        ...
        if block_tables is None:
            raise RuntimeError(
                "PreRoPEPolicy GPU-only decode requires block_tables for raw KV materialization"
            )
    ...
    def compute_chunked_prefill(
        ...
        _, positions, rotary_emb = self._get_rope_context()
        logger.debug(
            f"[DEBUG] PreRoPEPolicy.compute_chunked_prefill called, "
            f"layer={layer_id}, num_tokens={num_tokens}, "
    ...
    def compute_chunked_decode(
        ...
        if layer_id == 0:
            logger.debug(
                f"PreRoPE decode: selected_blocks={len(selected_blocks)}, "
