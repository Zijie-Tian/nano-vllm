...
class FullAttentionPolicy(SparsePolicy):
    ...
    def select_blocks(
        ...
        if ctx.layer_id == 0 and available_blocks:
            ...
            logger.debug(
                f"[Full] chunk={ctx.query_chunk_idx}, blocks={len(available_blocks)}, density=100.0%"
            )
    ...
    def print_density_stats(self) -> None:
        ...
        logger.info(
            f"[Full Policy] Density Stats: chunks={stats['num_chunks']}, "
    ...
    def compute_chunked_prefill(
        ...
        logger.debug(
            f"[DEBUG] FullPolicy.compute_chunked_prefill called, "
    ...
    def compute_chunked_decode(
        ...
        if layer_id == 0:
            logger.debug(
                f"Decode attention: selected_blocks={len(selected_blocks)}, seq.block_table={list(seq.block_table)}"
            )
        ...
        total_prefill_tokens = kvcache_manager.get_prefill_len(
            seq
        )  # Original prefill length
