...
### Step 2: 如不存在，使用 aliyunpan skill 下载

**必须通过 `Skill("aliyunpan")` 调用 aliyunpan skill 来执行下载操作。**

下载流程：
1. 调用 `Skill("aliyunpan")` 并说明需要下载的文件
2. 创建本地目录：`mkdir -p results/kvcache-rope/{model}/{length}`
3. 清除代理环境变量（skill 文档中有说明）
4. 设置 savedir、执行下载
5. 移动文件（aliyunpan 会创建嵌套目录 `{savedir}/{uid}/data/COMPASS/...`，需移动到 `{savedir}/layer_{xx}.pt`）
6. 清理嵌套目录
