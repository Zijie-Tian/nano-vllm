...
from .triattention_utils import (
    HeadFrequencyStats,
...
from .triattention_stats_utils import validate_stats_metadata
...
def apply_triattention_patch(
    ...
    *,
    ...
    def _cache_to_tuple(cache_obj, owner_model):
            ...
            if hasattr(cache_obj, "to_legacy_cache"):
                ...
                for item in legacy:
                    if not item or item[0] is None or item[1] is None:
                        has_none = True
                        continue
                    valid_legacy.append((item[0], item[1]))
                if valid_legacy and not has_none:
                    owner_model._triattention_cache_legacy_count = getattr(owner_model, "_triattention_cache_legacy_count", 0) + 1
                    return tuple(valid_legacy)
            if hasattr(cache_obj, "key_cache") and hasattr(cache_obj, "value_cache"):
                pairs = []
                for key_tensor, value_tensor in zip(cache_obj.key_cache, cache_obj.value_cache):
                    if key_tensor is None or value_tensor is None:
                        continue
                    pairs.append((key_tensor, value_tensor))
                if pairs:
                    owner_model._triattention_cache_fallback_count = getattr(owner_model, "_triattention_cache_fallback_count", 0) + 1
                    return tuple(pairs)
            return ()
        if not cache_obj:
            return ()
        ...
        if pairs:
            owner_model._triattention_cache_tuple_count = getattr(owner_model, "_triattention_cache_tuple_count", 0) + 1
        return tuple(pairs)
        ...
        **kwargs,
        ...
        if past_key_values is not None:
            if isinstance(past_key_values, Cache):
                ...
                if len(past_key_values) > 0 and past_key_values[0] and past_key_values[0][0] is not None and past_key_values[0][0].shape[2] > 0:
                    is_empty_cache = False
        ...
        if past_key_values is not None and input_ids is not None and not is_empty_cache:
            ...
            elif isinstance(past_key_values, (tuple, list)) and past_key_values and past_key_values[0] and past_key_values[0][0] is not None:
        ...
        pkv_tuple = _cache_to_tuple(outputs.past_key_values, self)
