...
def process_single_layer(args):
    ...
    for strip in q_strips:
        if strip.shape[2] < max_kv_width:
            padding = torch.zeros((strip.shape[0], strip.shape[1], max_kv_width - strip.shape[2]), 
                                 dtype=strip.dtype)
    ...
    for h in range(num_heads):
        ...
        im = ax.imshow(head_mask, cmap='viridis', aspect='auto', interpolation='nearest')
    ...
    fig.suptitle(f"BLASST Mask - Layer {layer_id} (All Heads)\nRows: Query Blocks, Cols: KV Subblocks", fontsize=16)
...
def plot_masks(source_dir, output_dir, workers):
    ...
    q_chunk_dirs = sorted([d for d in source_path.iterdir() if d.is_dir() and d.name.startswith("q_chunk_")],
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=workers) as executor:
        list(tqdm(executor.map(process_single_layer, tasks), total=len(tasks), desc="Rendering Maps"))

    print(f"Done! Combined maps saved to {output_dir}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--source", type=str, default="results/chunked_mask", help="Source directory")
    parser.add_argument("--output", type=str, default="results/chunked_mask_map", help="Output directory")
